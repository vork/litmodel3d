<script lang="ts">
	...
	export let env_map: string | FileData | null = null;
	export let tonemapping: string | null = null;
	export let exposure: number = 1.0;
	...
	let env_map_url: string;
	$: {
		env_map_url = env_map ? (typeof env_map === "string" ? env_map : env_map.url) : null
		if (env_map_url) {
			// Check if url starts with https:// or http://
			// If not, append the current url base path to the url
			if (!env_map_url.startsWith("https://") && !env_map_url.startsWith("http://")) {
				if (url) {
					let base_url = new URL(url).origin;
					env_map_url = base_url + '/file=' + env_map_url;
				}
			}
		}
	}
	...
	onMount(() => {
		...
		scene.clearColor = scene.clearColor = new BABYLON.Color4(
			...clear_color,
		);
	...
	function set_tonemapping(tonemap: string): void {
		if (scene) {
			if (tonemap) {
				if (tonemap === "standard") {
	...
	function set_exposure(exposure: number): void {
		if (scene) {
			scene.imageProcessingConfiguration.exposure = exposure;
		}
	}

	function set_contrast(contrast: number): void {
		if (scene) {
			scene.imageProcessingConfiguration.contrast = contrast;
		}
	}

	function load_env_map(url: string): void {
		if (url) {
			...
			console.log("Removing environment texture");
	...
	function create_camera(
		...
		const updateCameraSensibility = (): void => {
			helperCamera.wheelPrecision =
				250 / (helperCamera.radius * zoom_speed);
